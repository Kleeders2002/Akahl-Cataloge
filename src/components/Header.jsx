 ...
 * Layout: Sesión/Quote (izq) | Logo (centro) | Logout (der)
 */
    ...
    <header className="bg-gradient-to-b from-akahl-primary/95 to-akahl-primary/90 backdrop-blur-xl border-b border-akahl-secondary/30 sticky top-0 z-40 shadow-premium">
      ...
      <div className="max-w-6xl mx-auto px-4">
        ...
        <div className="flex items-center justify-between py-4">
          {/* Left side - Session time + Quote button */}
          <div className="flex items-center gap-4 flex-1">
            {/* Session time */}
            {elapsedTime && (
              <div className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-akahl-primary-dark/60 to-akahl-secondary/10 rounded-xl border border-akahl-secondary/20 shadow-premium">
                <span className="text-xs text-akahl-secondary/70 uppercase tracking-[0.15em]">Session:</span>
                <span className="text-sm font-bold text-akahl-secondary">{elapsedTime}</span>
              </div>
            )}

            {/* Quote button - only show in admin view */}
            {userRole === 'ADMIN' && currentView === 'admin' && (
              <button
                onClick={onBackToQuotation}
                className="hidden sm:flex items-center gap-2 px-4 py-2.5 bg-akahl-secondary/15 hover:bg-akahl-secondary/25 active:bg-akahl-secondary/35 rounded-xl transition-all active:scale-95 border border-akahl-secondary/40 shadow-premium hover:shadow-premium-lg"
              >
                <svg
            ...
            {/* Admin button - only show in quotation view */}
            {userRole === 'ADMIN' && currentView === 'quotation' && (
              <button
                onClick={onGoToAdmin}
                className="hidden sm:flex items-center gap-2 px-4 py-2.5 bg-akahl-secondary/15 hover:bg-akahl-secondary/25 active:bg-akahl-secondary/35 rounded-xl transition-all active:scale-95 border border-akahl-secondary/40 shadow-premium hover:shadow-premium-lg"
              >
